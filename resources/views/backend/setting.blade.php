...
<div class="card">
    ...
    @foreach($lang as $l)
      ...
      $k++;
      ?>
      <span><a href="setting/{{$l->id}}">{{$l->tag}}</a></span>
      @endforeach
      </h5>
    <div class="card-body">
    <form method="post" action="{{route('settings.update')}}">
        ...
        <div class="form-group">
          ...
          <input id="thumbnail" class="form-control" type="text" name="favicon" value="{{$data->favicon}}">
        </div>
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" rows="3" id="maps" name="maps">{{$data->maps}}</textarea>
          @error('maps')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>

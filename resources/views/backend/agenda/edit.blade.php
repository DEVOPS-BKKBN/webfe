...
@section('title','PORTAL-BKKBN || Agenda Edit')
@section('main-content')
<div class="card">
    <h5 class="card-header">Ubah Agenda</h5>
    <div class="card-body">
      <form method="post" action="{{route('agenda.update',$agenda->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="lang" class="form-control">
              ...
              @foreach($lang as $key=>$b)
                <option value='{{$b->id}}' {{(($agenda->lang==$b->id)? 'selected' : '')}}>{{$b->lang}}</option>
              @endforeach
          </select>
        </div>
        @php
        ...
        <div class="form-group">
            ...
            <select name="pejabat" class="form-control">
                <option value="{{$agenda->pejabat}}">{{$agenda->pejabat}}</option>
          ...
          <input id="inputTitle" type="text" name="title" placeholder="{{$agenda->title}}"  value="{{$agenda->title}}" class="form-control">
          @error('title')
          <span class="text-danger">{{$message}}</span>
          @enderror
          </div>
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" id="description" name="description">{{$agenda->description}}</textarea>
          @error('description')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="input-group">
          ...
          <input id="thumbnail" class="form-control" type="text" name="photo" value="{{$agenda->photo}}">
        </div>
        ...
        <div class="form-group">
          ...
          <input id="date" type="text" name="tanggal" placeholder="{{$agenda->tanggal}}" value="{{$agenda->tanggal}}" class="form-control" autocomplete="off">
          @error('tanggal')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
